...
exports.dashboard = async (req, res) => {
    ...
    const userNotesCountPromise = await userModel.findOne({email : req.user.email}); // number : notes array length
    const totalPages = Math.ceil(userNotesCountPromise['notes'].length / notesPerPage); // number : number of pages

    try {
        const sortedNotes = await userModel.aggregate([
...
exports.createNewNote = async (req, res) => {
    ...
    if (req.query.role === 'update') {
        noteToUpdate = await userModel.findOne({email : req.user.email}, 
            {
                notes : {
                    $elemMatch : {
                        _id : req.query.id
...
exports.addANewNote = async (req, res) => {
    ...
    try {
        ...
        const noteListUpdate = await userModel.updateOne({email : req.user.email},
            {
                $addToSet : {
                    notes : newNote
                }
...
exports.deleteNote = async (req, res) => {
    ...
    const update = await userModel.updateOne({email : req.user.email},
        {
            $pull : {
                notes : {
                    _id : noteId
...
exports.updateNote = async (req, res) => {
    ...
    try {
        ...
        const noteListUpdate = await userModel.updateOne(
